...
#define number_of_cache_lines 16384
#define v_type uint64_t
#define array_length 2

struct cacheline_t{
  uint64_t cacheline[array_length];
};
...
void read_non_cacheable_locations()
{
  ...
  if(mhartid == 0){
      ...
      core_local_variable = non_cached_cachelines[0].cacheline[0];
  }
  if(mhartid == 1){
      core_local_variable = 2;
      //variable_B = core_local_variable;
      core_local_variable = non_cached_cachelines[1].cacheline[0];
  }
}
